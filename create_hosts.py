...
parser.add_argument('--jmx', dest = 'dns', default = 'False', required = False, help = 'True ou False, adiciona interface JMX no lugar de SNMP, precisa especificar porta')
...
def manage_csv(cache=None,hosts=None):
    for host in hosts:
        ...
        try:
            ...
            #print(host['groups'])
            #host['ports'] = [ x.strip() for x in host['ports'].split(',') ]
...
def manage_host(cache=None,host=None):
    ...
    if host.get('ip', False):
        ...
        if args.dns == 'False':
            ...
            params['interfaces'] = [ {'type': '1', 'useip': '0', 'ip': '', 'main': '1', 'dns': host['dns'], 'port': '10050'}, {'type': '4', 'useip': '0', 'ip': '', 'main': '1', 'dns': host['dns'],'port': host['ports']} ]
    else:
        params['host'] = host['dns']
        params['interfaces'] = [ {'type': '1', 'useip': '0', 'dns': host['dns'], 'main': '1', 'ip': '', 'port': '10050'}, {'type': '4', 'useip': '0', 'dns': host['dns'], 'main': '1', 'ip': '','details' : {'version' : 2, 'bulk' : 1,'community' : '{$SNMP_COMMUNITY}'}, 'port': '161'} ]
    ...
    try:
        ...
        if params['macros'] == ' ':
            out = zapi.host.create(host=params['name'],interfaces=params['interfaces'],groups=params['groups'],proxy_hostid=params['proxy_hostid'],templates=params['templates'])
        else:
            out = zapi.host.create(host=params['name'],interfaces=params['interfaces'],groups=params['groups'],proxy_hostid=params['proxy_hostid'],macros=params['macros'],templates=params['templates'])
...
if args.file and args.filetype == 'json':
    ...
    hosts = list()
    with open("input_format/createHost_format.csv", mode='r') as csv_file:
