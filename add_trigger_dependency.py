...
parser.set_defaults(verbose=False, proxy='ALL', session_verify=False)
parser.add_argument('--trigger-proxy', dest='trigger', help='Enter initials of trigger name')
		...
		if host['host'] != proxy['host']:
			try:
				...
				for trigger_host in get_triggers_from_hosts:
					update_dependencies = []
					if len(trigger_host['dependencies']) == 0:
						alreadyDependant = False
					else:
						for dependency in trigger_host['dependencies']:
							if dependency['triggerid'] == trigger_id_proxy:
								alreadyDependant = True
								logger.info("found dependency on host {} [{}] with {} proxy.".format(host['host'],
																									 host['hostid'],
																									 proxy['host']))
							elif dependency['description'].startswith(args.trigger) and dependency[
								'triggerid'] != trigger_id_proxy:
								alreadyDependant = False
							elif dependency['description'].startswith('Proxy nodata 2m'):
								continue
							else:
					...
					if not alreadyDependant:
						try:
							logger.info(
								"Adding dependency to host {} [{}] for {} proxy.".format(host['host'], host['hostid'],
																						 proxy['host']))
							update_dependencies.append({'triggerid': trigger_id_proxy})
