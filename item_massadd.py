...
with open(args.file) as f:
	...
	for row in f_csv:
		try:
			zapi.item.create(name=row[0],key_="db.odbc.select["+row[8]+",{$ODBC_DSN}]" ,hostid=template_id,type=11,value_type=row[11],delay="0;"+row[9],params=row[6],history="1w",trends="456d",description=row[3],password="{$ODBC_PASSWORD}",username="{$ODBC_USER}")
